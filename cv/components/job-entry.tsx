...
export function JobEntry({ job, stackLabel }: { job: Job; stackLabel: string }) {
  return (
    <div className="mb-[22px] break-inside-avoid last:mb-0">
      ...
      <p className="m-0 mt-0.5 font-neue mb-3 text-sm tracking-wide leading-none normal-case text-(--cv-muted) italic">
        {job.dateRanges.join(' · ')}
      </p>
      <ul className="m-0 mb-3 list-disc pl-5">
        {job.bullets.map((bullet, i) => (
          <li key={i} className="mb-2 max-w-[58ch] font-light tracking-wide last:mb-0">
            {bullet}
          </li>
        ))}
      </ul>
